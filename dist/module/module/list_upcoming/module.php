<?php
	...
	function create_upcoming($pageType) {
		if ($pageType === 'trial') {
			$sql = "
				select trial_name as name, unit, start_dt, end_dt, trial_seq as seq
				from trial
				where
	    ...
	    while($row = $result->fetch_assoc()) {
	      ...
	      $html .= 
	      	"<tr> \n" .
	      	"  <td><a href=\"view.php?" . $module_urlSeq . "\">" . $row["name"] . "</a></td> \n" .
	      	"  <td>" . $row["unit"] . "</td> \n" .
	      	"  <td>" . date_format(date_create($row["start_dt"]), "m/d/Y") . "</td> \n" .
	      	"  <td>" . date_format(date_create($row["end_dt"]), "m/d/Y") . "</td> \n" .
	      	"  <td class=\"hidden-xs\" style=\"text-align:center;\"> \n" .
	      	"    <a href=\"view.php?" . $module_urlSeq . "\">View</a> \n" .
	      	"    | \n" .
	      	"    <a href=\"comment.php?" . $module_urlSeq . "\">Comment</a> \n" .
	      	"  </td> \n" .
	    		"</tr> \n";
	    }
	    $html .=
